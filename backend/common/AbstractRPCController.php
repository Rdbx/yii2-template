...
use common\exceptions\JsonRPCException;
use common\validators\jsonRPC\Value;
use common\validators\JsonRPCValidator;
...
class AbstractRPCController extends \Redbox\Core\AbstractController
{
    ...
    public function runAction($id, $params = [])
    {
        ...
        try {
            $requestObject = Json::decode(file_get_contents('php://input'),
                false);
        ...
        if (empty($requests)) {
            ...
            $resultData = [
                $this->formatResponse(null,
                    new JsonRPCException(Yii::t('yii', 'Invalid Request'),
        ...
        $response->headers->set("Content-Type",
            "application/json+rpc-2.0; charset=UTF-8");
        $response->data = json_encode($isBatch || null === $resultData
    ...
    private function getActionResponse($requestObject)
    {
        ...
        try {
            ...
            if ($e->getCode() === JsonRPCException::INVALID_PARAMS) {
                $error = new JsonRPCException($e->getMessage(),
                    JsonRPCException::INTERNAL_ERROR, $e->getData());
            } else {
                $error = $e;
            }
        } catch (\Exception $e) {
            $error = new JsonRPCException(Yii::t('yii', 'Internal error'),
        ...
        return $this->formatResponse($result, $error,
            isset($this->requestObject->id) ? $this->requestObject->id : null);
    }
    ...
    public function createAction($id)
    {
        ...
        if (empty($action)) {
            throw new JsonRPCException(Yii::t('yii', 'Method not found').' '
                .$id, JsonRPCException::METHOD_NOT_FOUND);
        }
     ...
     * @throws JsonRPCException if params are invalid
     */
    public function bindActionParams($action, $params)
    {
        try {
            ...
            $actionParams = [];
            $paramsTypes
                = $this->getMethodParamsTypes();//changes for array types documented as square brackets

            foreach ($method->getParameters() as $param) {
                $name = $param->getName();
                if (property_exists($params, $name)) {
                    if ($param->isArray()
                        || isset($paramsTypes[$name])
                        && strpos($paramsTypes[$name], "[]") !== false
                    ) { //changes for array types documented as square brackets
                        $args[]
                            = $actionParams[$name] = is_array($params->$name)
                            ? $params->$name : [$params->$name];
                    } elseif (!is_array($params->$name)) {
                        $args[] = $actionParams[$name] = $params->$name;
                    } else {
                        throw new JsonRPCException(Yii::t('yii',
            ...
            throw new JsonRPCException("Invalid Request",
                JsonRPCException::INVALID_REQUEST);
        }
    }
    ...
    {
        list($contentType) = explode(";",
            Yii::$app->request->getContentType()); //cut charset
        $headers = Yii::$app->request->getHeaders();
        if (!Yii::$app->request->getIsPost() || empty($contentType)
            || $contentType != "application/json+rpc-2.0"
        ) {
    ...
    private function parseAndValidateRequestObject($requestObject)
    {
        if (null === $requestObject) {
            throw new JsonRPCException(Yii::t('yii', 'Parse error'),
                JsonRPCException::PARSE_ERROR);
        }
        ...
        if (!is_object($requestObject)
            ...
            || empty($requestObject->method)
            || "string" != gettype($requestObject->method)
        ) {
            throw new JsonRPCException(Yii::t('yii', 'Invalid Request'),
                JsonRPCException::INVALID_REQUEST);
        }
    ...
    private function validateActionParams()
    {
        foreach ($this->requestObject->params as $name => $value) {
            ...
            foreach ($paramInfo['validators'] as $type => $params) {
                $paramValue = JsonRPCValidator::run($type, $params,
                    $paramValue);
            }
    ...
    {
        return array_reduce($this->methodInfo['params'],
    ...
    private function validateResult($result)
    {
        if (!empty($this->methodInfo['return'])) {
            $resultValue = new Value($this->methodInfo['return']['name'],
                $result, $this);
            foreach (
                $this->methodInfo['return']['validators'] as $type => $params
            ) {
                $resultValue = JsonRPCValidator::run($type, $params,
                    $resultValue);
            }
    ...
    private function parseMethodDocComment($method)
    {
        ...
        for ($i = 0; $i < count($lines); $i++) {
            preg_match("/@param\s+([\w\\\\\[\]]+)\s+$variableRegex/",
                $lines[$i], $paramMatches);
            ...
            if (!empty($subject) && !empty($validatorMatches)) {
                $subject['validators'][$validatorMatches[1]]
                    = trim($validatorMatches[2]);
            }
        }
    }
